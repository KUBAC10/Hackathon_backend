...
// service
import rangeQuestionReport from 'server/services/reportsBuilder/helpers/rangeQuestionReport';
...
describe('rangeQuestionReport', () => {
  ...
  describe('Text', () => {
    ...
    it('should return correct data', async () => {
      const res = await rangeQuestionReport({ surveyItem, question, timeZone, range });
      // expect structure
  ...
  describe('Split by hours', () => {
    ...
    describe('Checkboxes', () => {
      before(async () => {
        ...
        surveyItem = await surveyItemFactory({ team, survey, surveySection, question, range });
  ...
  describe('Split by days', () => {
    ...
    describe('Checkboxes', () => {
      ...
      it('should return correct data', async () => {
        ...
        const current = res.data.find(d => d.date.to.toISOString() === range.to).items;
        expect(current.every(d => d.value === 2)).to.be.eq(true);
        expect(current.every(d => questionItemIds.includes(d._id.toString()))).to.be.eq(true);
        // expect prev data
        expect(res.prevData.length).to.be.eq(0);
    ...
    describe('Thumbs', () => {
      ...
      it('should return correct data', async () => {
        ...
        // expect data
        expect(res.data.length).to.be.eq(6);
        const current = res.data.find(d => d.date.to.toISOString() === range.to);
        expect(current.items[0]._id).to.be.eq(0);
        expect(current.items[0].name).to.be.eq('yes');
        expect(current.items[0].value).to.be.eq(2);
        expect(current.items[1]._id).to.be.eq(1);
  ...
  describe('Split by weeks', () => {
    const range = {
      from: moment().subtract(2, 'weeks').startOf('day').toISOString(),
      to: moment().endOf('day').toISOString()
    };
  ...
  describe('Split by months', () => {
    const range = {
      from: moment().subtract(3, 'months').startOf('day').toISOString(),
      to: moment().endOf('day').toISOString()
    };
    ...
    describe('Checkboxes', () => {
      ...
      it('should return correct data', async () => {
        ...
        expect(res.prevData).to.be.an('array');
        // expect data
        expect(res.data.length).to.be.eq(4);
        const current = res.data.find(d => d.date.to.toISOString() === range.to).items;
        expect(current.every(d => d.value === 2)).to.be.eq(true);
        expect(current.every(d => questionItemIds.includes(d._id.toString()))).to.be.eq(true);
        // expect prev data
